...
RUN corepack enable
COPY . .
RUN pnpm install --allow-build=esbuild --allow-build=@tailwindcss/oxide --allow-build=@sentry/cli --allow-build=sharp --allow-build=workerd
...
ARG VITE_KEYGEN_VERSION=latest
RUN pnpm run build
